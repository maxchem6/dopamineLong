function results=peakAnalysisw(filtMat,i,timeVec10,ssimall,dPoint,calibCurr,currMinBound,c)
    ...
    for j=1:length(timeVec10)
        ...
        if indseed<=indlocold+0.8*wold 
            ...
            if indseed <= 10
                ...
                [curr,indloc,w,~]=findpeaks(itsmooth(1:indseed+200),'WidthReference','halfheight','MinPeakHeight',currMinBound,'MinPeakWidth',5,'NPeaks',1);
                curr=curr-min(itsmooth(1:indloc));
            
            elseif indseed <=c-200
                itsmooth=itsmooth-min([itsmooth(indseed-10:indseed+37) 0]);
                [curr,indloc,w,~]=findpeaks(itsmooth(indseed-1:indseed+200),'WidthReference','halfheight','MinPeakHeight',currMinBound,'MinPeakWidth',5,'NPeaks',1);
                curr=curr-min(itsmooth(indseed-4:indloc+indseed));   
                indloc=(indloc+indseed-2);
            else
                itsmooth=itsmooth-min([itsmooth(indseed-10:c) 0]);            
                [curr,indloc,w,~]=findpeaks(itsmooth(indseed-1:c),'WidthReference','halfheight','MinPeakHeight',currMinBound,'MinPeakWidth',5,'NPeaks',1);
            ...
            if size(curr)==1
                ...
                %if indloc-indseed > w
                    if indseed <= 10
                        ...
                        [curr,indloc,w,~]=findpeaks(itsmooth(1:indseed+150),'WidthReference','halfheight','MinPeakHeight',currMinBound,'MinPeakWidth',5,'NPeaks',1);
                        curr=curr-min(itsmooth(1:indloc));
            
                    elseif indseed <=c-150
                        itsmooth=itsmooth-min([itsmooth(indseed-10:indseed+18) 0]);
                        [curr,indloc,w,~]=findpeaks(itsmooth(indseed-1:indseed+150),'WidthReference','halfheight','MinPeakHeight',currMinBound,'MinPeakWidth',5,'NPeaks',1);
                        curr=curr-min(itsmooth(indseed-4:indloc+indseed));   
                        indloc=(indloc+indseed-2);
                    else
                        itsmooth=itsmooth-min([itsmooth(indseed-10:c) 0]);            
